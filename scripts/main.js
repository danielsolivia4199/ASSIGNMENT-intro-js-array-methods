...
const toggleCart = (event) => {
  if (event.target.id.includes("fav-btn")) {
   ...
   const index = referenceList.findIndex(input => input.id === Number(id))
...
const search = (event) => {
  ...
  const searchResult = referenceList.filter(input => 
    input.title.toLowerCase().includes(userInput) ||
    input.author.toLowerCase().includes(userInput) ||
    input.description.toLowerCase().includes(userInput)  
  )
...
// .reduce() & .some()
const cartTotal = () => {
  const cart = referenceList.filter(input => input.inCart);
  const total = cart.reduce((a,b) => a + b.price, 0)
  const free = cart.some(input => input.price <= 0);
